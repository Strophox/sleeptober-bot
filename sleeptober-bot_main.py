...
TOKEN_FILE = "sleeptober-bot_token.txt"
...
def compute_scoring(user_data):
    ...
    return (logged_total, hours_total / (logged_total or 1), hours_too_few, hours_too_many)
...
async def leaderboard(ctx):
    ...
    async with ctx.typing():
        ...
        if not data:
            ...
            scoring = [(user_id, compute_scoring(user_data)) for (user_id, user_data) in data.items()]
            scoring.sort(key=lambda t: (31 - t[1][0], t[1][2], t[1][3])) # (user_id, (logged_total, hours_average, hours_too_few, hours_too_many))
            embed.description += '\n'.join(f"{index+1}. `{f'-{hours_too_few:2.2f}': >6} {f'+{hours_too_many:2.2f}': >6}`, avg.{hours_average:.02f}h <@{user_id}> ({logged_total}d)" for index, (user_id, (logged_total, hours_average, hours_too_few, hours_too_many)) in enumerate(scoring))
